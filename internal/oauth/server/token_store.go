...
func NewTokenStore(cfg *entity.AuthConfig, db *gorm.DB) *TokenStore {
	store := &TokenStore{
		db:        db,
		tableName: entity.Token{}.TableName(),
		stdout:    os.Stderr,
...
func (s *TokenStore) gc() {
	for range s.ticker.C {
		...
		if count > 0 {
			// not soft delete.
			if err := s.db.Table(s.tableName).Where("expired_at <= ?", now).Or("code = ? and access = ? AND refresh = ?", "", "", "").Unscoped().Delete(&entity.Token{}).Error; err != nil {
				s.errorf("[ERROR]:%s\n", err)
			}
		}
	}
