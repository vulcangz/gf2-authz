...
type Base[T entity.Models] interface {
	...
	WithTransaction(transaction Transaction) *base[T]
}

// base struct that allows contacting the database using Gorm.
type base[T entity.Models] struct {
	db *gorm.DB
}

// New initializes a new repository.
func New[T entity.Models](db *gorm.DB) Base[T] {
	return &base[T]{
		db: db,
	}
}
...
func (r *base[T]) WithTransaction(transaction Transaction) *base[T] {
	return &base[T]{
		db: transaction.DB(),
	}
}
